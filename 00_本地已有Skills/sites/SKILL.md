...
description_en: Publish generated content or a local project as an online link, or unpublish an existing app. Use for websites, small games, static content, PDFs, or any project that can run as a single-port HTTP service, including Node.js, Python, and Go applications.
license: Internal
allowed-tools:
disable: false
---
...
## Choose the App Destination — Never Infer It from Directories

Local directories only tell the tool which files to upload. They do **not** prove whether the
user wants to update an existing app or create another app: agents may put several projects in
one directory, or put a later project in a subdirectory.

The first app in a workspace is created automatically. For later publishes, distinguish explicit
intent from an unspecified destination:

1. If the user's **latest message** explicitly asks to update the current/existing app or its live
   link after ordinary edits, call with `updateExistingApp: true`. When the workspace has exactly
   one app, the tool selects it directly and preserves its display name. With multiple apps it still
   returns `{"type":"sites_deploy_needs_app_selection"}` because the target remains ambiguous.
2. Otherwise call without `appId` / `createNewApp`. If the tool returns
   `sites_deploy_needs_app_selection`, list the candidates, ask whether to overwrite one existing
   app link or create a new app and link, then **end your turn and wait**.
3. If the user chooses an existing app, call again with its returned `appId`. When the current
   project is a different app that the user explicitly wants to use as the replacement, also pass
   `replaceExistingApp: true`; this updates the display name to the current project's `appName`.
   Leave that flag out for ordinary edits and re-publishes so the existing name does not drift.
4. If the user chooses a new app/link, call again with `createNewApp: true`.

`updateExistingApp` is not permission to guess: never set it for a generic publish request or a
different/new project, and never combine it with `createNewApp` or `replaceExistingApp`. Never
choose among multiple apps based on directory names, app-name similarity, or earlier conversation
context. Never pass `appId` and `createNewApp` together, and never infer `replaceExistingApp` from
a directory change.

### Apps activated via cloud service

An app that was already activated through `workbuddy_cloud_service` is a valid `appId` source. To
publish it, **reuse its `applicationId` as the sites tool's `appId`** — do **not** `createNewApp`.
Reusing the id keeps its reserved domain and cloud-service Origin; a brand-new app would get a
different domain and break its cloud login (the server enforces an exact Origin match).

## Multiple Apps in One Directory — Always Pass `entryHtml`

One directory can hold several independent apps (`createNewApp` uses a separate sandbox and link
even when the upload directory is the same). When that happens, the directory ends up with several
...
```json
{ "directory": "/abs/path/site", "createNewApp": true, "entryHtml": "b.html",
...
A single-app directory does not need it (the entry is auto-detected, `index.html` preferred), and
node/python/go projects never do (their entry is the HTTP root, not a file).
...
- `appId` (deploy only) — an existing app selected by the user from
  `sites_deploy_needs_app_selection`; publishing overwrites that app's current link.
- `createNewApp` (deploy only) — set to `true` only after the user selected “create a new app and
  link”; mutually exclusive with `appId`.
...
Publishing with an existing `appId` reuses that app's recorded sandbox when available, so its
share link stays the same and the content behind it is replaced. Publishing with
`createNewApp:true` uses a separate sandbox and link, even when the upload directory is the same.
Publishing with `appId` plus `replaceExistingApp:true` additionally updates the local and cloud
display name to the current project's `appName`; it does not change the appId or share link.
