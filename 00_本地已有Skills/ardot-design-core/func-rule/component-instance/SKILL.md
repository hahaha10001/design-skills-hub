...
For a remote team-library component, add `libraryKey` to the `ref` node. Use the component's source node id (`assetId` from `component_search`) as `ref`, and the library source file id as `libraryKey`. `libraryKey` is **only** for `batch_edit` instance refs — **never** pass it to `open_design` / `create_design`. Copy ids from `component_search` / live file data, never from this sample.
...
item4=I("35:2", {type: "ref", ref: "366:12", libraryKey: "<LibraryKey>", componentProperties: {"Show Icon#252:1": true, "Label#252:2": "Remote"}})
```
