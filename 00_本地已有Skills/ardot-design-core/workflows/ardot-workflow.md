...
> 4. **Validation tiers** — `[T1]`/`[T3]`/`[T4]`/`[T5]` tags mark which validation tier applies to each `batch_edit`. Do **not** run full screenshot+layout after every batch. `capture_screenshot` is scarce and vision-only: skip it if you cannot read images; at most 2 calls per section / repair batch. Cap corrective iterations at 2 per section.
...
Step 7: batch_edit → page frame + hero scaffold (structural)      [T1]
        ...
        → no screenshot; defer visual check to T4
        batch_edit → features section scaffold + content + style  [T4, section complete]
        → capture_layout(problemsOnly: true); capture_screenshot once if you can read images
        batch_edit → footer + CTA sections                        [T4, section complete]
        → capture_layout(problemsOnly: true); screenshot only if you can read images and quota remains
        IF any real issues accumulated → ONE batch_edit fixing all of them
        → re-run capture_layout; at most one more screenshot for the whole repair batch
        (Max 2 screenshot calls and 2 fix iterations per section; ignore ≤4px spacing noise.)

Step 8: capture_screenshot(full page)                            [T5, final; skip if T4 already covered the page, or if you cannot read images]
```
...
- Do not screenshot between T2 (pure content) or consecutive T3 batches — defer to the section boundary. Never screenshot → fix one issue → screenshot again.
...
Step 4: capture_screenshot → verify once after ALL substitutions  [T4]
        (Vision-only. Skip if you cannot read images. No capture_layout — substitutions don't change structure.)
```
...
Step 3: capture_layout(problemsOnly: true); capture_screenshot once if you can read images
Step 4: IF issues → ONE batch_edit fixing all → re-run capture_layout; at most one more screenshot (max 2 screenshot calls)
```
