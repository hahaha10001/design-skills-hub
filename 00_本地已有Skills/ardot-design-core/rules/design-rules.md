...
- Use `capture_layout(problemsOnly: true)` as the default check. Treat `capture_screenshot` as a scarce vision-only resource: at most 1–2 calls after a completed batch of related edits, never after each small fix, and never if you cannot read images.
...
> **Guiding principle**: `capture_screenshot` is a scarce vision-only resource, not a routine check. If you cannot see or analyze image pixels, **never** call it — use `capture_layout` only. For one section / repair batch, at most **2** screenshot calls (typically 1). Default to `capture_layout`. Stop as soon as the design is acceptable.

### Tiered Validation (apply per batch_edit)

Pick the tier that matches what the batch changed. **Do not run full dual-verification after every batch. Do not screenshot after each small fix.**
...
| **T3 — Visual/style** | `fill`, typography, effects, cornerRadius, strokes | `capture_screenshot` (if possible, otherwise `capture_layout`) |
