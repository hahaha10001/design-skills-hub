...
> ⛔ **HARD RULE — NEVER CALL MIORA TOOLS.** This is a 100% Ardot design task. Never call any Miora mcp tools. Always use ImageGen to generate images. NEVER call `miora_write_canvas` and `miora_open_canvas`, it's not allowed.
...
**On-canvas images — follow the injected directive.** The host may inject `<ardot_image_gen mode="ai|placeholder" />`. When present, obey it — do not deliberate:
- `mode="placeholder"` → labeled gray boxes only: `G(target, "placeholder", "<short label>")`. No real asset, no ImageGen, no image-generation budget.
- `mode="ai"` → generate images via **ImageGen**, then Upload onto the node. **NEVER** call `miora_xxx` tools.


When no `<ardot_image_gen>` directive is present: ImageGen for polished / final photography and hero art; `G("placeholder")` for drafts / wireframes or when ImageGen is unavailable. See the **Images** rules in `design-rules.md`.

**STRICT — one ImageGen (or one `G("placeholder")`) per image node.** Do not re-issue ImageGen on the same node to "fix" a screenshot that is still generating or looks slightly off — at most **one** retry on severe ImageGen failure. Continue layout/style with `U()` / other non-image ops. Re-issuing wastes budget and races the prior job.

### Step 8: Validate

Follow the **Post-Generation Validation Pattern** in `design-rules.md`. Use **tiered validation** — pick the lightest check that matches what the batch changed (T1 structural → `capture_layout` only; T2 content → skip; T3 visual → `capture_screenshot` (if possible, otherwise `capture_layout`); T4 section-complete → both once; T5 final page → one screenshot). **Do not run full dual-verification after every batch_edit.** Enforce the convergence threshold: **max 2 fix iterations per section**, ignore ≤4px spacing noise, no subjective re-polishing once the section matches spec.

## Screenshot Verification (mandatory, if possible)

If `capture_screenshot` is possible, use it and download the return image. These are internal verification artifacts — never surface their paths to the user, never write them elsewhere (`/tmp`, home dir, project root).
