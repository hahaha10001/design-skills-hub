...
尺寸映射适用于所有工作流；后续 prompt 与坐标表仅供风格参考图工作流使用。完整风格规则走语义排版，目录无合适匹配时走 `dsl-md-format.md`。
...
| 微信公众号首图 900×383 | 900×383 | `1536x768` |
| 自定义 W×H | 用户指定值 | 取最接近的支持尺寸 |

用户指定比例时直接使用该映射，不与风格文件原尺寸反复确认。

900×383 使用 `1536x768` 的 2:1 生图桶，最终在 900×383 画布中垂直居中裁切。
