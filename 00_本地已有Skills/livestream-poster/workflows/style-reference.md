...
用文件读取工具读取用户的风格参考图，输出一段可直接用于 ImageGen 的英文背景描述（3–6 句）。只提取：
...
只询问尚未提供、且无法从上下文可靠推断的信息。用 `AskUserQuestion` 收集仍缺失的项（人物与 Logo **分两题**，选项文案与跳过条件与 [style-selection.md](style-selection.md) Step 1 一致）。

⛔ **本步必须在 `create_design` / ImageGen 之前完成**。人物照片三选项必须包含 `我稍后上传`；prompt 中的讲师/嘉宾姓名不等于已上传照片。文案题只在用户完全没有可排版文字时出现；已有主题/讲师/时间/看点等关键信息时默认「其余 AI 补」，不得再问。任一字段选择「我稍后上传」或「我稍后提供完整文案」时，停止当前轮并等待补齐。
...
使用高保真人物参考。人物段禁止出现 vintage / retro / grain / silhouette / neon 等背景风格词。

### AI 生成人物

使用相同的背景/人物隔离规则，把人物锚点改为明确的外貌、服装和姿势描述，生图不带人物参考图。不得擅自推断真实人物身份或敏感属性。
