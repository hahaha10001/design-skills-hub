...
Extract design tokens, page structure and image assets from HTML/CSS. When the
input is HTML, linked stylesheets are fetched and merged automatically.

    python scripts/extract_design_tokens.py --file page.html
    python scripts/extract_design_tokens.py --url https://example.com
    cat page.html | python scripts/extract_design_tokens.py --stdin --base-url https://example.com
    python scripts/extract_design_tokens.py --file styles.css --format json
...
Dependencies: standard library only. That is the reason this one was adopted and
the Playwright crawler beside it upstream was not; this repo declares no Python
third-party dependencies and adding a browser download for one tool was not worth
it. For a site that renders entirely on the client, this extractor sees the
server HTML and little else, and the upstream crawler is the better instrument --
install it separately rather than vendoring it here.

Two limitations worth knowing before trusting the output:

  1. Colour detection is hex-centric. A stylesheet written in `oklch()`, `lab()`
     or `color-mix()` reports few or no colours even though it is full of them --
     the values still surface under CSS VARIABLES if they are custom properties,
     but the frequency ranking that picks a "primary" will be wrong. Increasingly
     common on exactly the well-built sites worth extracting from.
  2. Frequency is not hierarchy. The most-repeated colour is often a border, not
     a brand. Read the ranking as a census, not as a decision.

On the network path: `read_text_from_url` tries `urllib` first and falls back to
`curl -Ls` when TLS validation fails, which happens on systems with an incomplete
certificate chain. The fallback still validates certificates -- it uses the system
trust store rather than Python's -- and the URL is passed as an argument vector,
never through a shell. It does follow redirects, so point it at hosts you chose.
"""
